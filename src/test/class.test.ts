...
describe('Classes', () => {
    it('A single class selector', function () {
        const selector = '.a';
        expect(translate(selector)).to.eq(`An element with a class of 'a'`);
        expect(visualize(selector)).to.deep.eq([{ tag: 'div', classes: ['a'] }]);
    });
    it('A double class selector', function () {
        const selector = '.a.b';
        expect(translate(selector)).to.eq(`An element with classes 'a' and 'b'`);
    ...
    it('2 Separate class selectors', function () {
        const selector = '.a, .b';
        expect(translate(selector)).to.eq(`An element with a class of 'a' and an element with a class of 'b'`);
    ...
    describe('With elements', () => {
        it('A single element with one class', function () {
            const selector = 'p.a';
            expect(translate(selector)).to.eq(`A '<p>' element with a class of 'a'`);
        ...
        it('A single element with multiple classes (vowel prefix)', function () {
            ...
            expect(translate(selector)).to.eq(`An '<a>' element with classes 'a', 'b' and 'c'`);
